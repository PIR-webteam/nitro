...
(function ($, window, undefined) {
	...
	var iLightBox = function (el, options, items, instant) {
		...
		if(iL.options.linkId) {
			...
			iL.vars.WIN.iLightBoxHashChange(function(){
				iL.hashChangeHandler();
			});
		}
	...
	iLightBox.prototype = {
		...
		normalizeSocial: function(obj, url){
			...
			$.each(obj, function(key, value) {
				...
				obj[key] = {
					URL: value.URL && iL.absolutizeURI(baseURL, value.URL) || url && iL.absolutizeURI(baseURL, url),
					source: value.source || source || value.URL && iL.absolutizeURI(baseURL, value.URL) || url && iL.absolutizeURI(baseURL, url),
		...
		repositionPhoto: function(){
			...
			var thumbsOffsetW = (vars.isInFullScreen) ? 0 : ((path == 'horizontal') ? 0 : vars.thumbnails.outerWidth()),
		...
		doAjax: function(url, callback){
			var iL = this,
			url = "http://ilightbox.net/getSource/?url=" + encodeURIComponent(url).replace(/!/g, '%21').replace(/'/g, '%27').replace(/\(/g, '%28').replace(/\)/g, '%29').replace(/\*/g, '%2A'),
			xhr = $.ajax({
				url: url,
				dataType: 'json',
				cache: true
		...
		hashChangeHandler: function(url){
			...
			hash = iL.parseURI(URL).hash,
			index = hash.indexOf('#' + opts.linkId + '/'),
			split = hash.split('/');
			
			if(vars.hashLock) return;
		...
		absolutizeURI: function(base, href) {// RFC 3986
			...
			href = iL.parseURI(href || '');
			base = iL.parseURI(base || '');
